...
!!! tip "ページナビ"
    ◀️ 前 **[07. 入力処理](07-input.md)** ・ **次 ▶️ [09. 実際のバグと修正](09-debugging.md)**
...
42 の課題をここまで進めていれば **メモリリーク** は既にお馴染みの概念です。
ここでは **cub3D 固有の難しさ** を中心に整理します。

### メモリリーク（復習）

**`malloc` で確保したメモリを `free` し忘れて残り続けること**。
42 の評価では **検出された時点で即 0 点** なので、cub3D でも当然ゼロにします。
...
これまでの課題と違って、cub3D は **解放対象の種類が多い** です。
...
C++ のように「スコープを抜けたら自動で free」の仕組みは C にありません。
**エラーパス（`ft_error` で途中終了）でもちゃんと解放する** ための設計が必要です。

```
C++: デストラクタが自動で free してくれる
C:   エラー時も含めて自分で free
...
cub3D で採用している **「エラー発生時にどこまで確保したか」を追跡する仕組み**。
...
// エラーが起きたら…
ft_error("...");
// ↓ ft_error の中で errctx を見て解放
```

グローバル変数で持つのは C の古典的なパターン。
`ft_error` がどこから呼ばれても **確実に cleanup できる** ようにするためです。
