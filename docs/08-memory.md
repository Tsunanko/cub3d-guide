...
C++ には **RAII**（Resource Acquisition Is Initialization）という仕組みがあり、
デストラクタで自動解放されます。

**C にはこれがない** ので、**手動で全部解放** が必要です。

```
C++: デストラクタが自動で free してくれる
C:   エラー時も含めて自分で free

→ C のほうがミスしやすい (エラーパス漏れ)
```

### エラーコンテキスト（errctx）って何？

**「エラー発生時にどこまで確保してあるか」を追跡する仕組み** です。
...
次は [07. 実際のバグと修正](09-debugging.md) で、開発中に遭遇した生のバグと直し方を学びます。
